...
public class Auto extends Thread {
	...
	private double velocidade = 100;
	private ArrayList<DrivingData> drivingRepport;
	private DrivingData dados;
		...
		try {
			...
			if (!this.getSumo().isClosed()) {
				...
				DrivingData _repport = new DrivingData(
						...
						this.idAuto, this.driverID, System.currentTimeMillis()/1000, sumoPosition2D.x, sumoPosition2D.y,
				...
				//System.out.println(
				//		"getSpeedDeviation = " + (double) sumo.do_job_get(Vehicle.getSpeedDeviation(this.idAuto)));
				sumo.do_job_set(Vehicle.setSpeedMode(this.idAuto,31));
				sumo.do_job_set(Vehicle.setSpeed(this.idAuto, velocidade));
				...
				System.out.println("fechou pelo auto");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void setSpeedParar() throws Exception{
		velocidade = 0.0;
	}
	public void setSpeedAcelerar() throws Exception{
		velocidade = 100.0;
	}
